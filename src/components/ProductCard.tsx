...
import { FileSpreadsheet, MessageCircle, ArrowUpRight, Check, Shield } from 'lucide-react';
  ...
  return (
    <div className="group rounded-2xl bg-white border border-slate-200 hover:border-slate-300 transition-all duration-300 hover:shadow-xl flex flex-col overflow-hidden shadow-sm">
      ...
      <div className="relative h-56 w-full overflow-hidden bg-slate-100">
        ...
        {product.standard && (
          <div className="absolute bottom-3 right-3 z-10 max-w-[calc(100%-1.5rem)]">
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-semibold bg-white/95 text-slate-900 border border-slate-200/90 backdrop-blur-md shadow-md">
              <Shield className="w-3 h-3 text-slate-800 flex-shrink-0" />
      ...
      <div className="p-5 sm:p-6 flex-1 flex flex-col justify-between">
        <div>
          <h3 className="text-lg sm:text-xl font-bold text-slate-950 group-hover:text-slate-700 transition-colors mb-2.5 leading-snug">
            {product.name}
          </h3>
